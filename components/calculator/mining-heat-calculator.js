...
export function MiningHeatCalculator({ defaults }) {
  ...
  const resultRows = [
    {
      label: "Доход от майнинга",
      values: resultPeriods.map((period) => formatRub(revenueDay * period.multiplier)),
    },
    {
      label: "Затраты на электроэнергию",
      values: resultPeriods.map((period) => formatRub(powerCostDay * period.multiplier)),
    },
    {
      label: "Прибыль после электричества",
      values: resultPeriods.map((period) => formatRub(netDay * period.multiplier)),
    },
    {
      ...
      <div className="calculator-card calculator-card--split">
        <div className="calculator-panel calculator-panel--inputs">
          <div className="calculator-card__intro">
            ...
            <p>
              Введите основные числа: хешрейт, потребление, тариф и КПД утилизации
              тепла. Курсы можно проверить и изменить в блоке результатов.
            </p>
          </div>
          ...
          <p className="calculator-note">
            Для проекта мы отдельно считаем тариф, тип оборудования, сезонность и
            схему передачи тепла по контурам.
          </p>
        </div>
